...
// Queue Struct to contain timestamps for file changes on the system
#define TSIZE 10

__pragma(warning(push))
__pragma(warning(disable:4115))

//
//  Structure for managing current state.
//

struct ProcessThreatLevel {
	FILE_ID dwProcessId;			// Key 
	DWORD dwThreatLevel; 		// 
	DWORD dwIsWhiteListed;
	UT_hash_handle hh; 			// UTHash uses this to create hash mapping
};

struct HeuristicSignature {
	WCHAR CONST * wHeuristicPath;
	DWORD dwThreatLevel;
	UT_hash_handle hh;
};
...
int TrackProcess(struct ProcessThreatLevel ** ProcessMap, FILE_ID ProcessId, int ThreatLevel, int isWhiteListed);
...
int PreviousDriveCheck(int size, CHAR * previousDrives[30], WCHAR driveLetter[]);
...
extern WCHAR * Volume;
extern int IsPassive;
extern int IsVerbose;
extern int IsVeryVerbose;
