...
--*/
{
	...
	//relPath = "\\Device\\HarddiskVolume1\\Users\\Peter\\Desktop";
	//if (relevantPath(reparseRecordList->LogRecord.Name[0])) {
		SpyLog(recordList); // [Peter] Logs!
	//}
...
ENLIST_IN_TRANSACTION: 
    ...
    if (recordList) {
        ...
        // [PETER]
		//relPath = "\\Device\\HarddiskVolume1\\Users\\Peter\\Desktop";
		//if (relevantPath(nameToUse)) {
		SpyLog(recordList); // [PETER] Doesn't log?
		//}
    }
...
// Will return true (1) if the path to the file is relevant
// within our scope [PETER]
/*
int relevantPath(_In_ WCHAR CONST *loggedDir) {
	if (!strcmp(relPath, loggedDir)) {
		return 1;
	}
	else {
		return 0;
